...
class EldenRingLauncher(ctk.CTk):
    def __init__(self):
        ...
        self.attributes("-alpha", 0.05) # Start slightly visible to ensure user can see something even if animation lags
        ...
        self.game_dir = None
        self.config_path = os.path.join(self.launcher_base, "launcher_config.ini")
        self.load_config()
    ...
    def show_setup_view(self):
        ...
        ctk.CTkLabel(self.overlay, text="FIRST TIME SETUP", font=("Cinzel", 24, "bold"), text_color="#d4af37").pack(pady=(30, 10))
        ctk.CTkLabel(self.overlay, text="Elden Ring path not found.\nLet's locate your game files.", 
                      font=("Arial", 13), text_color="#aaaaaa").pack(pady=10)
        
        self.search_btn = ctk.CTkButton(self.overlay, text="Auto-Discover Game", command=self.start_auto_discovery,
                                         fg_color="#3e4a3d", hover_color="#4e5b4d", border_width=1, border_color="#d4af37")
        self.search_btn.pack(pady=20)
        
        ctk.CTkButton(self.overlay, text="Browse Manually...", command=self.manual_browse,
    ...
    def run_discovery(self):
        ...
        self.update_setup_status("Checking Registry...")
        try:
            import winreg
        ...
        self.update_setup_status("Checking Steam Libraries...")
        ...
        for drive in drives:
            self.update_setup_status(f"Checking Common Paths: {drive}...")
            common = [
        ...
        for drive in drives:
            self.update_setup_status(f"Searching {drive}...")
        ...
        self.update_setup_status("Search Finished.")
        self.after(0, self.check_discovery_end)

    def check_discovery_end(self):
        if not self.found_paths_set:
            self.search_btn.configure(state="normal", text="Auto-Discover Game")
            self.setup_status.configure(text="No installations found. Please browse manually.", text_color="#ff4444")
        else:
            self.setup_status.configure(text=f"Find total: {len(self.found_paths_set)} installations.", text_color="#d4af37")
    ...
    def init_selection_ui(self):
        ...
        ctk.CTkLabel(self.overlay, text="VERIFY INSTALLATION", font=("Cinzel", 20, "bold"), text_color="#d4af37").pack(pady=(20, 5))
        ctk.CTkLabel(self.overlay, text="Installations found so far.\nPlease click the correct one to confirm:", 
        ...
        self.setup_status = ctk.CTkLabel(self.overlay, text="Searching more...", font=("Arial", 11), text_color="gray")
        self.setup_status.pack(pady=5)
        
        ctk.CTkButton(self.overlay, text="Back / Retry", command=self.setup_ui).pack(pady=5)
    ...
    def manual_browse(self):
        ...
        path = filedialog.askdirectory(title="Locate Elden Ring 'Game' Folder")
        if path:
            if os.path.exists(os.path.join(path, "eldenring.exe")):
                self.complete_setup(path)
            else:
                self.setup_status.configure(text="Invalid folder. Must contain eldenring.exe", text_color="#ff4444")
        ...
        if folder_name.lower() != "game":
            ...
            try:
                ...
                if os.path.exists(new_path) and path.lower() != new_path.lower():
                    ...
                    self.show_rename_error(path, new_path, "A folder named 'Game' already exists in this location.")
                    return
    ...
    def show_rename_error(self, old_path, new_path, error_detail):
        ...
        ctk.CTkLabel(self.overlay, text="📂 FOLDER RENAME FAILED", font=("Cinzel", 22, "bold"), text_color="#ff4444").pack(pady=(30, 10))
        ...
        err_box.insert("0.0", f"Error: {error_detail}\n\nTarget: {new_path}")
        err_box.configure(state="disabled")
        
        ctk.CTkLabel(self.overlay, text="Common solutions:", font=("Arial", 13, "bold"), text_color="#d4af37").pack(pady=(10, 5))
        
        tips = [
            "• Close Elden Ring if it's currently running.",
            "• Close any Windows Explorer windows viewing this folder.",
            "• Ensure no other program is using files in this directory.",
            "• Try running this launcher as Administrator.",
            "• Check if a 'Game' folder already exists at the destination."
        ]
        ...
        ctk.CTkButton(btn_frame, text="Retry Rename", command=lambda: self.complete_setup(old_path),
                       fg_color="#3e4a3d", hover_color="#4e5b4d", width=150).pack(side="left", padx=10)
                       
        ctk.CTkButton(btn_frame, text="Back to Selection", command=self.show_setup_view,
    ...
    def show_bootstrap_ui(self, path):
        ...
        ctk.CTkLabel(self.overlay, text="BOOTSTRAPPING", font=("Cinzel", 24, "bold"), text_color="#d4af37").pack(pady=(30, 10))
        self.bootstrap_label = ctk.CTkLabel(self.overlay, text="Missing required launcher files.\nDownloading SPP bundle...", 
        ...
        self.debug_btn = ctk.CTkButton(self.overlay, text="Debug Info", command=self.show_debug_log, 
    ...
    def run_bootstrap(self, path):
        ...
        try:
            self.update_bootstrap_status("Downloading bundle...")
            ...
            if not download_success:
                try:
                    self.update_bootstrap_status("Retrying via BITS...")
            ...
            if not download_success:
                try:
                    self.update_bootstrap_status("Retrying via WebClient...")
            ...
            if not download_success:
                error_msg = "Download failed. Check your internet or proxy."
            ...
            self.update_bootstrap_status("Extracting files...")
            ...
            self.update_bootstrap_status("Bootstrap Complete!")
    ...
    def add_retry_button(self, path):
        ctk.CTkButton(self.overlay, text="Retry Download", command=lambda: self.show_bootstrap_ui(path),
                       fg_color="#3e4a3d", hover_color="#4e5b4d").pack(pady=10)
        ctk.CTkButton(self.overlay, text="Show Debug Log", command=self.show_debug_log,
                       fg_color="#444444", hover_color="#555555").pack(pady=5)
        ctk.CTkButton(self.overlay, text="Back", command=self.setup_ui).pack(pady=5)

    def show_debug_log(self):
        log_win = ctk.CTkToplevel(self)
        log_win.title("Bootstrap Debug Log")
    ...
    def show_main_view(self):
        ...
        self.mod_label = ctk.CTkLabel(self.mod_frame, text="Select Modpack:", font=("Arial", 12, "bold"), text_color="#d4af37")
        ...
        self.mod_selector = ctk.CTkSegmentedButton(self.mod_frame, 
                                                   values=["Vanilla", "Quality of Life", "Diablo Loot (RNG)"],
        ...
        ctk.CTkButton(self.tools_frame, text="Change Game Path", 
        ...
        ctk.CTkButton(self.tools_frame, text="Repair / Reinstall Files", 
    ...
    def on_modpack_change(self, value):
        self.save_config_value("modpack", value)
        self.update_status(f"Modpack set to {value}")
        if self.game_dir and not self.is_game_running():
            self.apply_modpack(value)

    def apply_modpack(self, pack_name):
        if not self.game_dir or not os.path.exists(self.game_dir):
            print(f"Apply Modpack Failed: No game directory found.")
            return
